  ...
  <div class="info">
    <div class="search">
      ...
      <div class="button-box">
        ...
        <el-button type="primary" @click="addInfo()">添加学生</el-button>
      </div>
    </div>
    ...
    <el-dialog :visible.sync="dialogFormVisible" width="30%" top="10vh"
      ...
      <student-form v-if="dialogFormVisible" :update="update" :add="add" :student="currentStudent"/>
    </el-dialog>
  </div>
</template>
...
import {delStudent, getAllStudent, delStudents, selectStudent} from "@/api/studentApi";
...
export default {
  ...
  data() {
    return {
      ...
      searchOptions: [
        ...
        {value: 'classNo', label: '班级'},
        {value: 'college', label: '学院'},
      ],
      // 设置table的列
      tableCol: [
        {prop: 'no', label: '序号', width: 50},
        {prop: 'sid', label: '学号'},
        {prop: 'name', label: '姓名'},
        ...
        {prop: 'college', label: '学院', width: 250},
      ],
      // table的基本操作
      operator: [
        {
  ...
  methods: {
    ...
    getInfo(student) {
      ...
      this.dialogFormVisible = true;
      // 触发父方法
      console.log("获取详情", student)
    },
    delInfo(student) {
      this.$confirm(`您确定要删除${student.sid}-${student.name}吗?`, '提示', {
    ...
    getAllStudent(queryParams) {
      if (!queryParams) {
        queryParams = this.queryParams;
      }
      getAllStudent(queryParams).then((res) => {
        if (res.status === 200) {
          return res.data;
        } else {
    ...
    batchDeletion() {
      if (this.handleSelection.length === 0 || this.handleSelection === []) {
        Msg.info("请选择想要删除的学生列");
        return;
      }
      this.$confirm(`您确定要删除这些学生吗?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        ...
        delStudents(delList).then(res => {
          if (res.status === 200) {
            Msg.success(res.message);
            this.searchParams.value = "";
            this.getAllStudent(this.queryParams);
          } else {
            Msg.error(res.message);
          }
        }).catch(err => Msg.error(err));
